...
SEXP HazardBs23L(SEXP x0, SEXP x, SEXP nph, SEXP timecat0, SEXP timecat, SEXP fixobs, SEXP param, SEXP paramf, SEXP deg, SEXP n, SEXP lw, SEXP matk, SEXP totk)
{
  ...
  if (nnph==1){
	...
	tempF += FixObs[i+t1]*ParamF[i];
      ...
      tempL0 += IntSpline23((*Fpt), TotKPos[tcz0-1], X0[z], &TotK[tcz0], &MatK[Cst2*tcz0], TempD, &MyParam[tcz0], N, lW, lleg);
	...
	for (j=1; j<nnph; j++){
      ...
      tempL = 0;
      for (i=0; i<tcz0; i++){
	tempL0 += IntSpline23((*Fpt), TotKPos[i-1], TotKPos[i], &TotK[i], &MatK[Cst2*i], TempD, &MyParam[i], N, lW, lleg);
      }
      for (i=0; i<tcz; i++){
