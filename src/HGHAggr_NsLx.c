...
/* spline, 2 times, w/ expected hazard)      */
/* Author: H. Charvat                        */
...
SEXP HGHAggr_NsLx(SEXP x0, SEXP x, SEXP nph, SEXP timecat0, SEXP timecat, SEXP fixobs, SEXP statobs, SEXP lambdaobs, SEXP nbyclust, SEXP param, SEXP paramf, SEXP deg, SEXP n, SEXP lw, SEXP matk, SEXP totk, SEXP intk, SEXP nsadj1, SEXP nsadj2)
{
  ...
  double tempF, tempLH, tempH, tempL, tempL0, tempDH, tempTotH, tempGH, tempHH, expTF, NexpTF, NiNeTF, NlNeTF;
  double Total = 0;
  int z = 0;
  double TempD[6];
  ...
  for (nc=0; nc<nclust; nc++){
	...
	tempL += IntDNSplH(IntK[i], IntK[i+1], &TotK[i], &MatK[4*i], NsAdj1, NsAdj2, MyBasisB, TempD, MyParam, N, lW, lleg, leB, nbase, (i+firstK), tempLvec, tempHess, Res);
      ...
      tempLH = DeltaNSpl(X[z], &TotK[tcz], &MatK[4*tcz], NsAdj1, NsAdj2, MyBasisB, TempD, MyParam, leB, nbase, (tcz+firstK), Res);
	...
	for (j=0; j<nbase; j++){
	  tempGLH[nfix + i*nbase+j] = Res[j]*Nph[i+t2];
