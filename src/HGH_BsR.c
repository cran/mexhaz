...
SEXP HGH_BsR(SEXP x, SEXP nph, SEXP timecat, SEXP fixobs, SEXP param, SEXP paramf, SEXP deg, SEXP n, SEXP lw, SEXP matk, SEXP totk)
{
  ...
  for (z=0; z<lx; z++){
    ...
    for (i=0; i<nnph; i++){
      ...
      for (j=0; j<nbase; j++){
	GradLHaz[nfix + i*nbase+j][z] = Res[j+1]*Nph[i+t2];
