...
% \VignetteIndexEntry{excesshazard}
...
\title{\pkg{mexhaz}: An \proglang{R} Package for Fitting Flexible Hazard-Based
  Regression Models for Overall and Excess Mortality with a Random Effect}

%% for pretty printing and a nice hypersummary also set:
\Plainauthor{Hadrien Charvat, Aur\'elien Belot} %% comma-separated
\Plaintitle{mexhaz: An R Package for Fitting Flexible Hazard-Based
  Regression Models for Overall and Excess Mortality with a Random Effect} %% without formatting
\Shorttitle{mexhaz: Mixed-Effect Excess Hazard Regression Models} %% a short title (if necessary)

%% an abstract and keywords
\Abstract{This vignette provides a description of the \proglang{R}
  package \pkg{mexhaz}. It is based on an article accepted for
  ...
  package and compare the results obtained with those of other
